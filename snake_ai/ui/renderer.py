...
class SnakeRenderer:
    ...
    def _draw_grid_content(self, state: GameState, offset_x: int, offset_y: int):
        ...
        for i, pt in enumerate(state.snake):
            ...
            pygame.draw.rect(
                self.display, color,
                pygame.Rect(offset_x + pt[0] * block, offset_y + pt[1] * block, block - 1, block - 1)
            )

        # Dessiner la nourriture
        pygame.draw.rect(
            self.display, (255, 0, 0),
            pygame.Rect(offset_x + state.food[0] * block, offset_y + state.food[1] * block, block, block)
        )

        # Dessiner les bordures de la grille
        grid_rect = pygame.Rect(offset_x, offset_y, state.grid_width * block, state.grid_height * block)
    ...
    def render_duel(self, state_a: GameState, state_rl: GameState):
        ...
        score_a = self.small_font.render(f"Score: {state_a.score} | Pas: {state_a.steps}", True, (200, 200, 200))
        ...
        score_rl = self.small_font.render(f"Score: {state_rl.score} | Pas: {state_rl.steps}", True, (200, 200, 200))
